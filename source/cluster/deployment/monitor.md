...
实验室集群因在寒假期间使用内网穿透等原因被挖矿病毒攻击过，为了能及时发现服务器的异常情况搭建了实验室集群基于[prometheus](http://prometheus.dscl.team), [alertmanager](http://alertmanager.dscl.team), [grafana](http://grafana.dscl.team)的监控系统。本文文档主要用于介绍服务的部署和备份。
...
1. 在实验室集群各个节点安装 node exporter: master, host2, host3, host4, host5, host6。
   ...
   2. 撰写 service 文件开机自启动，`systemctl enable prometheus_node_exporter`。
      ```bash
...
## 备份
监控服务 node_exporter 上报的数据会被 prometheus 存储起来，这个目录暂时没有挂载出来，如果需要备份，需要对这个目录进行挂载。监控主要是为了发现集群近期的问题，暂时觉得没有必要对过去数据进行保留，如果服务出现问题，重新按照安装部署步骤进行部署即可。
